...
d :diario
"""
ihh_a = ii_tt.heliotermico_huglin("a", tb=10)
ihh_ar = ii_tt.heliotermico_huglin("ar", tb=10)
ihh_d = ii_tt.heliotermico_huglin("d", tb=10)
lons = ihh_a.lons; lats=ihh_a.lats
print(np.min(ihh_a.values), np.max(ihh_a.values))
...
df = iibb.extraer_indice_punto_grilla(ihh_a, df_aws, nombre_iibb="ihh_01_21sep1999")

# Guardar en archivo excel
#------------------------------------------------------------------------
#df.to_excel(img_path+"excel/ihh_01_21sep1999.xlsx", header=True,index=False)

# Graficar serie de tiempo en punto de grilla
#-------------------------------------------------------------------------------
img_serie_tiempo = dict(data = ihh_d,
                        punto_grilla = [-76.77, -6.28],
                        titulo_derecha = "01-21 SETIEMBRE 1999",
                        titulo_izquierda = "ÍNDICE BIOCLIMÁTICO: ESTACIÓN PACAYZAPA",
                        nombre_ejey = "(°C)",
                        img_path = img_path,
                        img_save_name = "ihh_d_stiempo_01_21setiembre1999_pacayzapa",
                       )
...
#------------------------------------------------------------------------
df = df.loc[34:,:].sort_values(by='ihh_01_21sep1999')
#print(df.loc[0:10,:])
#print(df.size)
img_hlineas = dict(data=df,
                   nombre_var = "ihh_01_21sep1999",
                   rango_val = [0,400],
                   nombre_ejex="(°C)",
                   titulo_derecha="01-21 Setiembre 1999",
                   titulo_izquierda ="ÍNDICE BIOCLIMÁTICO",
                   img_path = img_path,
                   img_save_name="ihh_a_hlines_aws_1999_san_mantin",
                   )
#iiibb.graficar_hlineas(img_hlineas)

# graficar mapa de indice 
#------------------------------------------------------------------------
img_mapa = dict(data=ihh_a,
                titulo_derecha="01-21 SETIEMBRE 1999",
                titulo_izquierda ="ÍNDICE BIOCLIMÁTICO",
                cb_rango= [0, 550, 50],
                tipo_cbar=None,
                cb_nombre="Indice heliotermico huglin (°C)",                
                img_path = img_path,
                shp_path = "/home/data/shape/depart_san_martin/",
                img_save_name="ihh_a_map_01_21sep1999_san_mantin",
                )
...
#------------------------------------------------------------------------
dic = {"data": ihh_a,
       "nombre_guardar": "ihh_01_21set1999.tif",
       }
